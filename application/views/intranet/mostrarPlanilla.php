<?php 
$TAMANO_PAGINA = count($hoy);
$total_paginas=count($hoy);
$pagina = @$_GET["pagina"];
if (!$pagina) {
    $inicio = 0;
    $pagina=1;
                ...
                echo "<table class='table table-hover-striped' style='text-align:left;' >
                       <thead>
                        ...
                        if ($total_paginas > 1){
                           for ($i=1;$i<=$total_paginas;$i++){
                              if ($pagina == $i){  $_SESSION['per']=$i;
                                          if(count($hoy[($i-1)])==0){
                                            //echo "nada";
                                            $bool=false;
                                          } else $bool=true;
                                        foreach ($hoy[($i-1)] as $aula) {
                                          ?>
                                            <tr>
                                                <td><?= $aula->periodo;  ?></td>
                                                <td><?= $aula->sala ;?></td>
                    ...
                    <?php 
                      if($aula->estado=='t'){//t de true
                        ...
                        ?>
                        <td><span class='col-sm-10 label label-danger'>Suspendida</span></td>
                        <td>Si <br><input type="radio" href="javascript:void(0);"onclick="habilitar('<?php base_url()?>habilitar/<?php echo $aula->pk; ?>')"></td>
                        <td>No <br><input type="radio"  checked name="asiste_'<?php echo $aula->pk;?>"></td>
                        <?php 
                      }
                      ?>
                      </tr>
...
<script>
  $(document).ready(function(e) {                                        
        $('input[rel="abrir"]').click(function(e) {
                            e.preventDefault();
                 
                var periodo = $(this).attr('periodo');// aca capturo en la variable temp lo que estaba en title
                var sala = $(this).attr('sala');
                var nombre = $(this).attr('nam');
                var asign = $(this).attr('asig');
                var secc = $(this).attr('secc');
                var pk = $(this).attr('pkr');

                //y si queres podes enviarla de vuelta a la pagina, fuera de la ventana modal:
                document.getElementById("per").innerHTML = periodo; 
                document.getElementById("sala").innerHTML = sala; 
                document.getElementById("nombre").innerHTML = nombre; 
                document.getElementById("asign").innerHTML = asign; 
                document.getElementById("secc").innerHTML = secc; 
                document.getElementById("pkr").value = pk; 


            });
    });
    //aca le envio el valor de la variable temp al input de html con id org.
</script>
      ...
      <div class="modal-content">
       ...
       <!-- <form action="" role="form" class="form-horizontal">-->
              ...
              <div class="form-group">    
                  ...
                  <div class="col-sm-3">
                  <?php $att= array('onsubmit'=>'return validacion()'); echo form_open('intranet/suspencion',$att); ?>
...
<script>
  function habilitar(url) {
    if (confirm("¿Realmente desea habilitar este registro?")) {
        window.location = url;
